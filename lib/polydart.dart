...
import 'src/types/market.dart';
import 'src/types/clob.dart';
...
class MarketDetailFetcher {
  ...
  static Future<BundleField<List<TradeRecord>>> _fetchTrades(
    ...
    for (final TradeRecord trade in allTrades) {
      if (seen.add(trade.transactionHash)) {
        deduped.add(trade);
      }
    }
    deduped.sort(
...
final class Polydart {
  ...
  /// Fetches all detail-screen data for a market in one call.
  ///
  /// Equivalent to calling the Gamma API for the market, CLOB for order
  /// books and price history, and the CLOB public-trades endpoint, all with
  /// per-field error resilience so a missing order book does not prevent
  /// market metadata or trades from loading.
  ///
  /// See [MarketDetailFetcher.fetch] for the standalone version.
  Future<MarketDetailBundle> marketBundle({
    required String conditionId,
    List<String>? tokenIds,
    String priceHistoryInterval = '1h',
  }) {
    return MarketDetailFetcher.fetch(
      gamma: gamma,
      clob: clob,
